...
export default function Quimicos() {
  ...
  useEffect(() => {
    api.get('/api/productos?categoria=Químicos')
      .then((res) => {
        setProductosQuimicos(res.data);
        setLoading(false);
      })
      .catch((err) => {
        console.error('Error al cargar productos Químicos:', err);
        setLoading(false);
      });
  }, []);
    ...
    <MainLayout>
      <Box sx={{ px: 4, py: 6, maxWidth: '1200px', mx: 'auto' }}>
        ...
        <Breadcrumbs separator="›" sx={{ mb: 3 }}>
          ...
          <Typography color="text.primary">Químicos</Typography>
        </Breadcrumbs>
        ...
        <Typography variant="body1" sx={{ color: 'text.secondary', mb: 5 }}>
          Soluciones para nutrición, control y limpieza en cultivos tropicales 🌿🧪
        </Typography>

        {/* Tarjetas */}
        <Grid container spacing={4}>
          {!loading && productosQuimicos.length === 0 && (
        ...
        <Box sx={{
          ...
          <Typography variant="body2" sx={{ mb: 3 }}>
            Nuestro equipo puede ayudarte a elegir el químico ideal para tu cultivo, según clima, etapa y tipo de suelo 🌱
          </Typography>
          <Button
            variant="contained"
            color="success"
